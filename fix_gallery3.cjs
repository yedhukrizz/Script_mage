...
const stateOld = `  const [bulkEffect, setBulkEffect] = useState<string>('');
  const [bulkEffect2, setBulkEffect2] = useState<string>('');
  const [bulkEffect3, setBulkEffect3] = useState<string>('');`;

const stateNew = `  const [bulkEffect, setBulkEffect] = useState<string>('');`;

content = content.replace(stateOld, stateNew);

const applyOld = `    if (!bulkFont && !bulkColor && !bulkEffect && !bulkEffect2 && !bulkEffect3) {
...
const applyNew = `    if (!bulkFont && !bulkColor && !bulkEffect) {
      addToast('Select at least one property to apply', 'info');
      return;
    }
...
const bulkSelectOld = `          <div className="flex-1 min-w-[150px] px-2 py-1 flex flex-col justify-center">
...
const bulkSelectNew = `          <div className="flex-1 min-w-[200px] px-2 py-1 flex flex-col justify-center">
