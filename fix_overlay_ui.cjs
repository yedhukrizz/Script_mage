...
const targetStr = `{activeSubMenu === 'overlay' && (
...
const newStr = `{activeSubMenu === 'overlay' && (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-3">
                ...
                {gridOverlay !== 'none' && (
                  <div className="flex flex-col gap-2 p-3 bg-[var(--theme-input-bg)] border border-panel-border rounded-xl">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <label className="w-6 h-6 rounded-full relative overflow-hidden cursor-pointer shadow-sm border-2 border-transparent hover:scale-110 transition-transform">
...
// To fix if target string is not exactly matched, let's use regex or a robust replace:
if (!content.includes("Canvas Overlays") || content.includes(newStr)) {
    console.log('Already replaced or failed');
} else {
    fs.writeFileSync('src/components/Toolbar.tsx', content);
    console.log('Successfully replaced overlay UI');
}
