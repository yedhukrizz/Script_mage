...
const effectsOld = `        const effects = [element.textEffect, element.textEffect2, element.textEffect3].filter(Boolean) as string[];
        const textEffectClass = effects.filter(e => e !== 'none').map(e => \`effect-\${e}\`).join(' ');`;

const effectsNew = `        const effects = [element.textEffect].filter(Boolean) as string[];
        const textEffectClass = element.textEffect && element.textEffect !== 'none' ? \`effect-\${element.textEffect}\` : '';`;

content = content.replace(effectsOld, effectsNew);

const newRenderOld = `        const staticEffectsClass = effects.filter(e => e !== 'none' && !['shiver', 'flicker', 'glitch', 'wave'].includes(e)).map(e => \`effect-\${e}\`).join(' ');
...
const newRenderNew = `        return isEditingText ? (
          <textarea
            ref={textInputRef as any}
            value={element.content}
            onChange={(e) => updateElement(element.id, { content: e.target.value })}
...
content = content.replace(newRenderOld, newRenderNew);
